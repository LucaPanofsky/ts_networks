...
test("error when network is empty", () => {
  const result = handleRun({ source: geometrySrc, network: "", cells: {} });
  expect(result.ok).toBe(false);
});

test("error when network not found", () => {
  const result = handleRun({ source: geometrySrc, network: "missing", cells: {} });
  expect(result.ok).toBe(false);
});

test("error on bad cell expression", () => {
  const result = handleRun({ source: geometrySrc, network: "rectangleMetrics", cells: { rect: "???" } });
  expect(result.ok).toBe(false);
});

// ── geometry evaluation ───────────────────────────────────────────────────────

test("evaluates rectangleMetrics: area = width * height", () => {
  const result = handleRun({
    source: geometrySrc,
...
test("evaluates rectangleMetrics: area = 0 for zero width", () => {
  const result = handleRun({
    source: geometrySrc,
    network: "rectangleMetrics",
