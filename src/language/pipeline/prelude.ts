...
//
// `PRELUDE_SOURCE` (REUSED, exported from the old sandbox) is the prelude written IN THE
// LANGUAGE as ordinary `defn`s — `not`/`and`/`or`, `add`/`sub`/`mul`/`div`, the comparison
// family, and propagatable wrappers over the `math/*` intrinsics. Because slice 4 makes the
// new pipeline compile `defn`, we DOGFOOD the prelude through our own splitter+parser rather
// than re-encode it: the prelude becomes ordinary `FnNode`s, emitted like any user function.
//
// Shadowing: a user definition of the same name WINS — we drop the prelude entry, mirroring
// `src/sandbox/prelude.ts:withPrelude`. Injection happens at EMIT time only, so `parseProgram`
// keeps reporting exactly the user's AST (parse/typecheck/diagram see only what was written).
//
...
import { PRELUDE_SOURCE } from "../../sandbox/prelude.js";
