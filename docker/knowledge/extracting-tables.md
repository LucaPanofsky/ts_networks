...
[`defgrammar`](defining-grammars.md): where a grammar recognises prose, a `TTable` reads
...
free-running prose, reach for a [grammar](defining-grammars.md) instead.
...
```bash
# doc=@ seeds the raw text of a /workspace file (read verbatim, never evaluated):
tsn-run my-table.tsn extractPairs doc=@sample.txt
```
...
— so it can be the **leaf of a [`defextract`](extracting-documents.md)** anywhere a scan
...
[`examples/treaty_table/treaty_total.tsn`](examples/treaty/treaty_total.tsn).
...
- Composing tables into a tree: [extracting documents](extracting-documents.md).
- Writing prose recognisers instead: [defining grammars](defining-grammars.md).
- Runtime: [`src/sandbox/ttable-runtime.ts`](/app/ts-networks/src/sandbox/ttable-runtime.ts)
  (`compileTTable`, `validateTTable`).
- Worked example (composition): [`examples/treaty_table/treaty_total.tsn`](examples/treaty/treaty_total.tsn).
