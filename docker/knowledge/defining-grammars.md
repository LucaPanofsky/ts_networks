...
[`examples/gdpr_article_structured_extraction.tsn`](examples/gdpr/gdpr_article_structured_extraction.tsn).
...
2. **Run it** on the real text. Seed the document straight from `/workspace` with `doc=@`
   (raw text, never evaluated — no `jq` quoting hack needed):
   ```bash
   tsn-run <file.tsn> <network> doc=@sample.txt
   ```
   Iterate: a sample that *should* match but doesn't tells you which rule to grow. When the
   captured record looks right on every shape your document shows (and on a negative you
   expect to *not* match), the grammar is done.
...
- Worked examples in our conventions: [`examples/citations.tsn`](examples/citations.tsn),
  [`examples/gdpr_article_structured_extraction.tsn`](examples/gdpr/gdpr_article_structured_extraction.tsn)
- The capture/parse/scan runtime: [`src/sandbox/grammar-runtime.ts`](/app/ts-networks/src/sandbox/grammar-runtime.ts)
